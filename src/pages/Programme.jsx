...
function sessionStyle(type) {
  ...
  if (t.includes('endurance') || t.includes('footing') || t.includes('ef') || t.includes('récup')) return { chip: 'chip-ef', color: 'var(--ef-color)', bg: 'var(--ef-bg)', label: 'EF' }
  if (t.includes('fractionné') || t.includes('vma') || t.includes('interval')) return { chip: 'chip-frac', color: 'var(--frac-color)', bg: 'var(--frac-bg)', label: 'Fractionné' }
  if (t.includes('longue') || t.includes('long')) return { chip: 'chip-long', color: 'var(--long-color)', bg: 'var(--long-bg)', label: 'Sortie longue' }
  if (t.includes('seuil') || t.includes('tempo')) return { chip: 'chip-seuil', color: 'var(--seuil-color)', bg: 'var(--seuil-bg)', label: 'Seuil' }
  if (t.includes('côte') || t.includes('cote') || t.includes('montée')) return { chip: 'chip-cotes', color: 'var(--cotes-color)', bg: 'var(--cotes-bg)', label: 'Côtes' }
  return { chip: 'chip-repos', color: 'var(--repos-color)', bg: 'var(--repos-bg)', label: type || 'Séance' }
}

// Sécurise l'affichage — convertit les objets en string si l'IA retourne un mauvais format
function safeStr(val) {
  if (val === null || val === undefined) return ''
  if (typeof val === 'string') return val
  if (typeof val === 'number') return String(val)
  if (typeof val === 'object') {
    if (val.pace_km) return String(val.pace_km) + '/km'
    if (val.pace) return String(val.pace) + '/km'
    if (val.fc && val.fc_max) return val.fc + '-' + val.fc_max + ' bpm'
    if (val.fc) return String(val.fc) + ' bpm'
      ...
      <div className="card fade-in" style={{ width:'100%', maxWidth:480, maxHeight:'80vh', overflowY:'auto', boxShadow:'var(--shadow-lg)' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
          <div>
            ...
            <p style={{ fontSize:12, color:'var(--text2)', marginTop:2 }}>Séance: <strong>{safeStr(session.title)}</strong></p>
          </div>
          ...
          {runs.map(run => {
            ...
            return (
              <div key={run.id} onClick={() => setSelected(selected?.id===run.id?null:run)}
                style={{ padding:'11px 14px', borderRadius:10, cursor:'pointer', transition:'all 0.15s', border:`2px solid ${selected?.id===run.id?'var(--accent)':'var(--border)'}`, background:selected?.id===run.id?'var(--accent-light)':'var(--bg3)' }}>
    ...
    <div style={{ marginTop:12, padding:'12px 14px', background:'var(--bg3)', borderRadius:10, border:'1px solid var(--border)' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
          ...
          <span style={{ fontSize:11, color:'var(--text2)' }}>{safeStr(analysis.quality_label)}</span>
        </div>
      </div>
      <div style={{ marginBottom:8 }}>
        ...
        <div className="progress-bar"><div className="progress-fill" style={{ width:`${analysis.compliance_pct}%`, background:analysis.compliance_pct>80?'var(--ef-color)':analysis.compliance_pct>60?'var(--long-color)':'var(--accent)' }}/></div>
      </div>
      {analysis.pace_analysis && <p style={{ fontSize:11, color:'var(--text2)', padding:'5px 8px', background:'var(--bg2)', borderRadius:6, marginBottom:6 }}>🏃 {safeStr(analysis.pace_analysis)}</p>}
      {analysis.heart_rate_analysis && <p style={{ fontSize:11, color:'var(--text2)', padding:'5px 8px', background:'var(--bg2)', borderRadius:6, marginBottom:6 }}>♥ {safeStr(analysis.heart_rate_analysis)}</p>}
      {analysis.positives?.map((p,i) => <p key={i} style={{ fontSize:11, color:'var(--ef-color)', marginBottom:3 }}>✓ {safeStr(p)}</p>)}
      {analysis.improvements?.map((p,i) => <p key={i} style={{ fontSize:11, color:'var(--frac-color)', marginBottom:3 }}>→ {safeStr(p)}</p>)}
      {analysis.coach_comment && (
        <div style={{ padding:'8px 10px', background:'var(--accent-light)', borderRadius:8, borderLeft:'3px solid var(--accent)', marginTop:8 }}>
          <p style={{ fontSize:11, color:'var(--text)', lineHeight:1.5 }}>💬 {safeStr(analysis.coach_comment)}</p>
        </div>
      )}
      {analysis.next_session_adjustment && <p style={{ fontSize:11, color:'var(--long-color)', fontWeight:600, marginTop:6 }}>📌 {safeStr(analysis.next_session_adjustment)}</p>}
    </div>
  )
}
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.45)', display:'flex', alignItems:'flex-end', justifyContent:'center', zIndex:200 }}>
      <div className="fade-in" style={{ background:'var(--bg2)', borderRadius:'20px 20px 0 0', width:'100%', maxWidth:640, maxHeight:'90vh', overflowY:'auto', padding:'24px', boxShadow:'var(--shadow-lg)' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:20 }}>
          <div>
            <span className={style.chip} style={{ padding:'3px 10px', borderRadius:100, fontSize:11, fontWeight:700 }}>{style.label}</span>
            <h2 style={{ fontSize:22, fontWeight:800, marginTop:8, color:'var(--text)' }}>{safeStr(session.title)}</h2>
        ...
        {(session.target_pace || session.heart_rate_zone) && (
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:18 }}>
            {session.heart_rate_zone && <div style={{ background:'var(--bg3)', borderRadius:12, padding:'12px 14px' }}>
              ...
              <p style={{ fontSize:14, fontWeight:700, color:'var(--text)' }}>{safeStr(session.heart_rate_zone)}</p>
            </div>}
            {session.target_pace && <div style={{ background:style.bg, borderRadius:12, padding:'12px 14px', border:`1px solid ${style.color}30` }}>
              <p style={{ fontSize:10, color:'var(--text3)', textTransform:'uppercase', letterSpacing:0.08, marginBottom:4 }}>Allure cible</p>
              <p style={{ fontSize:14, fontWeight:700, color:style.color }}>{safeStr(session.target_pace)}</p>
            </div>}
          </div>
        )}
        ...
        {session.warmup && <div style={{ marginBottom:14 }}>
          ...
          <p style={{ fontSize:13, color:'var(--text)', lineHeight:1.6 }}>{safeStr(session.warmup)}</p>
        </div>}
        {session.main_set && <div style={{ marginBottom:14 }}>
          ...
          <p style={{ fontSize:13, color:'var(--text)', lineHeight:1.6, background:'var(--bg3)', padding:'12px 14px', borderRadius:10, borderLeft:`3px solid ${style.color}` }}>{safeStr(session.main_set)}</p>
        </div>}
        {session.cooldown && <div style={{ marginBottom:14 }}>
          ...
          <p style={{ fontSize:13, color:'var(--text)', lineHeight:1.6 }}>{safeStr(session.cooldown)}</p>
        </div>}
        {session.tips && <div style={{ padding:'10px 12px', background:'#fffbeb', borderRadius:10, border:'1px solid #fde68a', marginBottom:14 }}>
          <p style={{ fontSize:12, color:'#d97706', lineHeight:1.5 }}>💡 {safeStr(session.tips)}</p>
        </div>}
        {session.nutrition_tip && <div style={{ padding:'10px 12px', background:'var(--accent3-light)', borderRadius:10, marginBottom:14 }}>
          <p style={{ fontSize:12, color:'var(--accent3)' }}>🍌 {safeStr(session.nutrition_tip)}</p>
        </div>}
        ...
        {session.strava_activity && (
          <div style={{ padding:'8px 12px', background:'#fff5f0', border:'1px solid #ffd0b0', borderRadius:8, marginBottom:14, display:'flex', gap:8, alignItems:'center' }}>
            ...
            <span style={{ fontSize:12, fontWeight:600, color:'#FC4C02' }}>{safeStr(session.strava_activity.name)}</span>
...
export default function Programme() {
  ...
  const { stravaData, stats } = useStrava()
  ...
  const handleRegenerate = async () => {
    ...
    try {
      const newPlan = await generateTrainingPlan(
        { race_type:goal.race_type, distance_km:goal.distance_km, elevation_gain:goal.elevation_gain, race_date:goal.race_date, level:goal.level, sessions_per_week:goal.sessions_per_week, preferred_days:goal.preferred_days, current_weekly_km:goal.current_weekly_km, target_time:goal.target_time },
        stats
      )
  ...
  return (
    <div className="fade-in">
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:20, flexWrap:'wrap', gap:12 }}>
        <div>
          <h1 style={{ fontFamily:'var(--font-display)', fontSize:32, letterSpacing:1 }}>{safeStr(planData.plan_name)}</h1>
        ...
        <div style={{ display:'flex', gap:6, marginBottom:16, overflowX:'auto', paddingBottom:4 }}>
          {planData.phases.map((phase, i) => (
            <div key={i} style={{ padding:'5px 12px', borderRadius:100, whiteSpace:'nowrap', background:'var(--bg2)', border:'1px solid var(--border)', fontSize:11 }}>
              <span style={{ color:phase.color||'var(--accent)', fontWeight:700 }}>{safeStr(phase.name)}</span>
            ...
            <div key={week.week_number} style={{ background:'var(--bg2)', border:`2px solid ${isCurrentWeek?'var(--accent)':'var(--border)'}`, borderRadius:20, overflow:'hidden', boxShadow:isCurrentWeek?'0 0 0 4px rgba(255,85,0,0.07)':'var(--shadow-sm)' }}>
                ...
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                  <div>
                    <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:3 }}>
                      ...
                      {isCurrentWeek && <span style={{ fontSize:9, fontWeight:800, padding:'3px 8px', borderRadius:100, background:'var(--accent)', color:'#fff' }}>EN COURS</span>}
                      <span style={{ fontSize:11, color:'var(--text3)', fontStyle:'italic' }}>{safeStr(week.phase)}</span>
                    </div>
                    <p style={{ fontSize:13, color:'var(--text2)' }}>{safeStr(week.focus)}</p>
                  </div>
                  <div style={{ textAlign:'right' }}>
                    ...
                    {totalCount>0 && <p style={{ fontSize:11, color:completedCount===totalCount?'var(--ef-color)':'var(--text3)', fontWeight:700 }}>{completedCount}/{totalCount} ✓</p>}
                  </div>
                </div>
                {totalCount>0 && (
                  <div className="progress-bar" style={{ marginTop:10 }}>
                    <div className="progress-fill" style={{ width:`${(completedCount/totalCount)*100}%`, background:completedCount===totalCount?'var(--ef-color)':'var(--accent)' }}/>
                  </div>
                )}
              ...
              <div style={{ padding:'16px 20px', display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:8 }}>
                {DAYS.map((dayLabel, dayIdx) => {
                  ...
                  const st = daySessions.length > 0 ? sessionStyle(daySessions[0]?.type) : null

                  return (
                    <div key={dayIdx}
                      onClick={() => {
                        if (daySessions.length>0) {
                          const sIdx = (week.sessions||[]).findIndex(s => s === daySessions[0])
                          setSelectedSession({ session:daySessions[0], weekIdx, sessionIdx:sIdx })
                        }
                      }}
                      style={{ minHeight:90, borderRadius:12, background:isToday?'var(--accent-light)':daySessions.length===0?'var(--bg3)':'var(--bg2)', border:`1.5px solid ${isToday?'var(--accent)':st?st.color+'40':'var(--border)'}`, padding:8, transition:'all 0.15s', cursor:daySessions.length>0?'pointer':'default', position:'relative' }}>
                      ...
                      {daySessions.length>0 ? daySessions.map((s,i) => {
                        const sty = sessionStyle(s.type)
                        return (
                          <div key={i} style={{ background:sty.bg, border:`1px solid ${sty.color}30`, borderRadius:8, padding:'5px 6px', marginBottom:4 }}>
                            <p style={{ fontSize:9, fontWeight:800, color:sty.color, textTransform:'uppercase', letterSpacing:0.05, marginBottom:2 }}>{sty.label}</p>
                            <p style={{ fontSize:9, color:'var(--text2)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{safeStr(s.title)}</p>
                            <p style={{ fontSize:9, color:'var(--text3)', marginTop:2 }}>{s.distance_km>0?`${s.distance_km}km`:`${s.duration_min}min`}</p>
                            {s.completed && <div style={{ position:'absolute', top:6, right:6, width:14, height:14, borderRadius:'50%', background:'var(--ef-color)', display:'flex', alignItems:'center', justifyContent:'center' }}><CheckCircle size={9} color="#fff"/></div>}
                          </div>
                        )
                      }) : (
      ...
      {planData.race_day_plan && viewMode==='complet' && (
        <div className="card" style={{ marginTop:24, borderLeft:'4px solid var(--accent)' }}>
          ...
          <p style={{ fontSize:13, color:'var(--text)', lineHeight:1.6 }}>{safeStr(planData.race_day_plan)}</p>
        </div>
      )}
